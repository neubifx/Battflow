...
GROMACS simulations are executed through a dedicated wrapper (GromacsWrapper), in which a modified class adapts job submission to different HPC schedulers and execution environments as specified by the `md_run_env` configuration in the `default.yaml` file which can be simple called by `battflow --write-config`. ORCA calculations are orchestrated via the Atomic Simulation Environment (ASE) calculator interface. Within this file whiich is copied in the main folder, `Battflow` default settings can be changed  to adapt to different environments and installations, e.g. different `host`, `database` and `collections` in MongoDB (including Atlas)

No molecular dynamics or density functional theory parameters are hard-coded within `Battflow`. Instead, simulation settings are defined explicitly by the user through native GROMACS input files stored in the resources directory and through the dft_simulations section of config/default.yaml. This design allows users to fully customise simulation protocols without modifying the workflow source code.

To enable computationally ready data management, `Battflow` is designed for direct integration with MongoDB, supporting both local and cloud-based installations depending on the internet accessibility of the target HPC environment. The workflow is orchestrated entirely in Python within an Anaconda environment, allowing straightforward installation of supporting tools such as ACPYPE for AMBER force-field generation. Python-based analysis libraries, including MDAnalysis and Solvation Structure, are used for post-processing and data extraction, while ORCA calculations are orchestrated via the Atomic Simulation Environment (ASE) calculator interface.
